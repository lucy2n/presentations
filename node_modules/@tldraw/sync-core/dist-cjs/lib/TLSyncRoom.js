...
class TLSyncRoom {
  ...
  broadcastPatch({ diff, sourceSessionId }) {
    this.sessions.forEach((session) => {
      ...
      if (!res.ok) {
        this.rejectSession(
          session,
          res.error === import_store.MigrationFailureReason.TargetVersionTooNew ? import_protocol.TLIncompatibilityReason.ServerTooOld : import_protocol.TLIncompatibilityReason.ClientTooOld
        );
        return;
      }
  ...
  handleConnectRequest(session, message) {
    ...
    if (theirProtocolVersion == null || theirProtocolVersion < (0, import_protocol.getTlsyncProtocolVersion)()) {
      this.rejectSession(session, import_protocol.TLIncompatibilityReason.ClientTooOld);
      return;
    } else if (theirProtocolVersion > (0, import_protocol.getTlsyncProtocolVersion)()) {
      this.rejectSession(session, import_protocol.TLIncompatibilityReason.ServerTooOld);
      return;
    }
    ...
    (0, import_state.transaction)((rollback) => {
      if (
        ...
        if (!migrated.ok) {
          ...
          this.rejectSession(
            session,
            migrated.error === import_store.MigrationFailureReason.TargetVersionTooNew ? import_protocol.TLIncompatibilityReason.ServerTooOld : import_protocol.TLIncompatibilityReason.ClientTooOld
          );
          return;
        }
        connect({
  ...
  handlePushRequest(session, message) {
    ...
    (0, import_state.transaction)((rollback) => {
      ...
      const addDocument = (changes, id, _state) => {
        ...
        if (res.type === "error") {
          return fail(
            res.reason === import_store.MigrationFailureReason.TargetVersionTooOld ? import_protocol.TLIncompatibilityReason.ServerTooOld : import_protocol.TLIncompatibilityReason.ClientTooOld
          );
        }
        ...
        if (doc) {
          ...
          if (!diff.ok) {
            return fail(import_protocol.TLIncompatibilityReason.InvalidRecord);
          }
          if (diff.value) {
      ...
      const patchDocument = (changes, id, patch) => {
        ...
        if (downgraded.type === "error") {
          return fail(import_protocol.TLIncompatibilityReason.ClientTooOld);
        }
        ...
        for (const [id, op] of Object.entries(message.diff)) {
          switch (op[0]) {
            ...
            case import_diff.RecordOpType.Remove: {
              ...
              if (!this.documentTypes.has(doc.state.typeName)) {
                return fail(import_protocol.TLIncompatibilityReason.InvalidOperation);
              }
      ...
      if (
        ...
        if (session) {
          ...
          if (!migrateResult.ok) {
            return fail(
              migrateResult.error === import_store.MigrationFailureReason.TargetVersionTooNew ? import_protocol.TLIncompatibilityReason.ServerTooOld : import_protocol.TLIncompatibilityReason.ClientTooOld
            );
          }
          this.sendMessage(session.sessionId, {
            type: "push_result",
